...
switch($ts){
	case "":
		...
		$title = 'Сообщения '.$strUser['username'];
	...
	case "do":
		...
		if($content == ''){
			tsNotice('Сообщение не может быть пустым!');
		}
		...
		$msg_content = 'Кто-то оставил вам сообщение, почтите его!';
	...
	case "redo":
		...
		$msg_userid = '0';
		$msg_touserid = $touserid;
		$msg_content = 'Кто-то оставил вам сообщение, прочтите его!';
        $msg_tourl = tsUrl('user','guestbook',array('id'=>$touserid));
		aac('message')->sendmsg($msg_userid,$msg_touserid,$msg_content,$msg_tourl);
		tsNotice('Ответ отправлен!');
		break;
	...
	case "delete":
		...
		tsNotice('Сообщение успешно удалено!');
		break;
}
