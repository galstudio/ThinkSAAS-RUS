...
class MySql {
	...
	function __construct($DB) {
		...
		if (! function_exists ( 'mysqli_connect' )) {
			qiMsg ( 'Сервер PHP не поддерживает базу данных MySQLi' );
		}

		$this->conn = mysqli_connect ( $DB ['host'],$DB ['user'], $DB ['pwd'] ,$DB ['name'],$DB ['port'] );

		if(mysqli_connect_errno())qiMsg('Ошибка связи с базой данных/база данных не найдена: '. mysqli_connect_error());
